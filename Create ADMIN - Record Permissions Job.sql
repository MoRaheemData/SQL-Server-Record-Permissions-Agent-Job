...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'ADMIN - Record Permissions', 
		...
		@notify_email_operator_name=N'DBA Chicago', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
begin
	exec [security_audit].[USP_AlertOnPermissionsChanges] ''dist.it.jf.dbachicago@dciinc.org'';
end
else
begin
	return
